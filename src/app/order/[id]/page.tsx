...
// Mock order data (replace with Convex query later)
const MOCK_ORDER = {
    id: "MVL-20251202-00147",
    date: "2 December 2024",
    time: "14:32 EAT",
    paymentMethod: "cod", // "mpesa" or "cod"
...
// Status timeline for Pay on Delivery
const STATUS_STEPS = [
    { id: "confirmed", label: "Order Confirmed", icon: CheckCircle, time: "Today, 14:32" },
...
export default function OrderConfirmationPage({ params }: { params: { id: string } }) {
    ...
    // Clear cart on mount (order confirmed)
    useEffect(() => {
        clearCart();
    }, []);
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
                    ...
                    <div className="bg-gray-900 text-white p-6 text-center">
                        ...
                        <p className="text-xs text-gray-300 mt-1">KEBS-Certified PPE Supplier</p>
                        <p className="text-xs text-gray-400 mt-2">Nairobi, Kenya | +254 700 123 456</p>
                    </div>
                    ...
                    <div className="p-6 border-b border-dashed border-gray-300">
                        <div className="grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div className="text-right">
                                <p className="text-gray-500 text-xs uppercase tracking-wider">Date & Time</p>
                    ...
                    {/* Payment Badge */}
                    <div className={`p-4 ${order.paymentMethod === "cod" ? "bg-emerald-50 border-b border-emerald-200" : "bg-orange-50 border-b border-orange-200"}`}>
                        <div className="flex items-center gap-2">
                            <div className={`flex items-center justify-center h-6 w-6 rounded-full ${order.paymentMethod === "cod" ? "bg-emerald-500" : "bg-orange-500"} text-white text-xs font-bold`}>
                                {order.paymentMethod === "cod" ? "✓" : "M"}
                            </div>
                            <div>
                                <p className="text-sm font-semibold text-gray-900">
                                    {order.paymentMethod === "cod" ? "Pay on Delivery" : "M-Pesa Payment"}
                                </p>
                    ...
                    {/* Shipping Details */}
                    <div className="p-6 border-b border-dashed border-gray-300">
                        <h3 className="font-bold text-gray-900 mb-3 text-sm uppercase tracking-wider">Delivery Address</h3>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b-2 border-gray-300">
                                    ...
                                    <th className="text-center py-2 font-bold text-gray-700 w-12">Qty</th>
                                    <th className="text-right py-2 font-bold text-gray-700 w-20">Total</th>
                                </tr>
                            </thead>
                            <tbody>
                                {order.items.map((item, idx) => (
                                    <tr key={idx} className="border-b border-gray-200">
                                        ...
                                        <td className="py-3 text-right font-semibold text-gray-900">KES {item.total}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
